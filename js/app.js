...
function formatMinutesAsDecimalHours(value) {
  const minutes = Number(value || 0);
  return minutes ? (minutes / 60).toFixed(2) : '0.00';
}
    ...
    modalRoot.querySelector('[data-record-form]').addEventListener('submit', async (event) => {
      ...
      config.fields.forEach((field) => {
        ...
        if (field.storeAsMinutes && payload[field.name] !== '') {
          const minutes = parseTimeSpanToMinutes(payload[field.name]);
          payload[field.name] = String(minutes !== null ? minutes : 0);
        }
...
function applyFormEnhancements(config, modalRoot) {
  ...
  if (config.sheet === 'Attendance') {
    ...
    const otMinutes = getFormField(modalRoot, 'OTMinutes');
    const OT_LIMIT_MINUTES = 660;
    let manualOvertime = false;
    ...
    const recalc = () => {
      ...
      const actualMinutes = Math.max(0, diff(inTime, outTime) - breakMinutes);
      const regularMinutes = Math.min(actualMinutes, OT_LIMIT_MINUTES);
      const autoOvertime = Math.max(0, actualMinutes - OT_LIMIT_MINUTES);
      const enteredOvertimeMinutes = parseTimeSpanToMinutes(otMinutes?.value);
      const useManualOvertime = manualOvertime && enteredOvertimeMinutes !== null && String(otMinutes?.value || '').trim() !== '';
      const finalOvertimeMinutes = useManualOvertime ? enteredOvertimeMinutes : autoOvertime;

      if (workMinutes) workMinutes.value = (regularMinutes / 60).toFixed(2);
      if (otMinutes) otMinutes.value = formatMinutesAsDecimalHours(finalOvertimeMinutes);
    };
    otMinutes?.addEventListener('input', () => {
      manualOvertime = true;
    });
